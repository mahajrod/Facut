		...
		while ((l_forward = kseq_read(seq_forward)) >= 0) {
            ...
            */
			...
			if ((mean_score_forward >= quality_threshold) &&
				(mean_score_reverse >= quality_threshold))// && (retain_forward == 1) && (retain_reverse == 1))
			{
                ...
                */
				...
				if ((mean_score_forward >= quality_threshold) &&
					(mean_score_reverse < quality_threshold))// && (retain_forward == 1))
				{
					...
					if ((mean_score_forward < quality_threshold) &&
						(mean_score_reverse >= quality_threshold))// && (retain_reverse == 1))
					{
